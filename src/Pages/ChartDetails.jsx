...
const SingleTune = ({ cover, title, isSingle, time, id, artist }) => {
    ...
    return (
        <button className='w-full bg-[#33373b5e] rounded-xl flex gap-4 p-2.5 items-center' id={id} onClick={()=> setPlaying(songInfo)}>
...
const ChartDetails = () => {
    ...
    return (
        <div className='mb-40'>
            <img src={playlistInfo?.cover || lead} className='w-full' alt="" />
            <h2 className='text-[#A4C7C6] text-4xl font-bold mt-6'>{playlistInfo?.title || "Tomorrow’s tunes"}</h2>
            <p className='des text-[#99933d] text-sm mt-2 mb-2.5'>{playlistInfo?.info || "Lorem ipsum dolor sit amet, consectetur adipiscing elit ut aliquam, purus sit amet luctus venenatis"}</p>
            <p>
                <span>{playlistInfo?.files?.length || "64"} songs - </span>
                <span className='pl-1'>{length || " 0"} hrs+</span>
            </p>
            <div className='w-full gap-2 grid mt-6' style={{ gridTemplateColumns: 'auto auto auto' }}>
